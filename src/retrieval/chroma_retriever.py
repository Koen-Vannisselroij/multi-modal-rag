...
class ChromaTextRetriever(BaseTextRetriever):
    ...
    def retrieve_texts(self, query: str, top_k: int = 3, max_distance: float = 1.5) -> list:
        ...
        distances = results.get("distances", [[]])[0] if "distances" in results else [None for _ in documents]
        output = []
...
class ChromaImageRetriever(BaseImageRetriever):
    ...
    def retrieve_images(self, query: str, top_k: int = 3, max_distance: float = 1.5):
        ...
        documents = results.get("documents", [[]])[0]
        distances = results.get("distances", [[]])[0] if "distances" in results else [None for _ in documents]
        output = [(doc, dist) for doc, dist in zip(documents, distances) if dist is not None and dist < max_distance]
        if not output:
