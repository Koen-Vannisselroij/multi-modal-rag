...
def main():
    ...
    if not docs and not image_docs:
        logger.error("No documents or images found in data/. Please add .txt files or images.")
        return
    ...
    while True:
        ...
        filtered_text_results = [(result, distance) for result, distance in text_results if distance is not None and distance < THRESHOLD]
        if filtered_text_results:
            for result, distance in filtered_text_results:
        ...
        filtered_image_results = [(doc, distance) for doc, distance in image_results if distance is not None and distance < THRESHOLD]
        if filtered_image_results:
            for doc, distance in filtered_image_results:
